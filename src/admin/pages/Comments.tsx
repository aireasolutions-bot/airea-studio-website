...
import { mergePages } from "@/lib/pages";
...
export function Comments() {
  ...
  // Dynamic page list: the manifest + any page that already has comments.
  const pages = useMemo(() => mergePages(comments.map((c) => c.page)), [comments]);
  const pageMeta = pages.find((p) => p.slug === page) ?? pages[0];
  ...
  handlers.current.selectPin = (id: string) => {
    setSelected(id);
    if (fullscreen) setDrawerOpen(true);
  ...
  // scaled preview math (mirrors the editor — desktop renders as real desktop)
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {fullscreen && (
            <button
              ...
              title="Comments"
              className={cn("relative grid h-8 w-8 place-items-center rounded-lg", drawerOpen ? "bg-blue-mist text-blue-ink" : "text-ink-3 hover:bg-ink/5 hover:text-ink")}
            >
              <MessagesSquare className="h-4 w-4" />
              {openCount(page) > 0 && (
                <span className="absolute -right-1 -top-1 grid h-4 min-w-[16px] place-items-center rounded-full bg-blue px-1 text-[9px] font-bold text-white">{openCount(page)}</span>
              )}
            </button>
          )}
          <button
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex rounded-full border border-line-2 bg-white p-0.5">
          {SEG.map((s) => (
            <button
              ...
              className={cn("rounded-full px-3 py-1 text-[12px] font-semibold transition-colors", filter === s.id ? "bg-blue text-white" : "text-ink-2 hover:text-ink")}
            >
              {s.label}
            </button>
          ))}
        ...
        <button
          ...
          className={cn("rounded-full border px-3 py-1 text-[12px] font-semibold transition-colors", mineOnly ? "border-blue bg-blue-mist text-blue-ink" : "border-line-2 text-ink-2 hover:text-ink")}
        >
          @ me
        </button>
      </div>
  ...
  return (
    <div>
      ...
      {fullscreen ? (
        <div className="fixed inset-0 z-[70] bg-ink/70 p-3 backdrop-blur-sm md:p-4">
          <div className="relative h-full">
            {preview}
            <div
              className={cn(
                "absolute inset-y-0 right-0 z-10 w-[380px] max-w-[82vw] overflow-y-auto rounded-2xl border border-line bg-canvas p-3 shadow-card transition-transform duration-300",
                drawerOpen ? "translate-x-0" : "translate-x-[112%]"
              )}
            >
              {panel}
            </div>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_390px]">
          <div className="lg:sticky lg:top-24 lg:h-[calc(100vh-8rem)]">{preview}</div>
          <div>{panel}</div>
        </div>
      )}
    </div>
